...
########################################################################
#  Changes adapted by Pinpoint and relicensed under the same license   #
#                                                                      #
#  Adds support for:                                                   #
#                                                                      #
#  - golang                                                            #
...
ENV PATH="/usr/local/go/bin:/go/bin:/usr/local/node-v${NODEJS_VERSION}-linux-x64/bin:${PATH}"

RUN set -ex \
    && cd /usr/local \
    && curl --silent -L https://storage.googleapis.com/golang/go${GO_VERSION}.linux-amd64.tar.gz | tar xz \
    && cd / \
    && go version \
...
RUN set -ex \
    && curl --silent -L https://github.com/stedolan/jq/releases/download/jq-${JQ_VERSION}/jq-linux64 -o /bin/jq \
    && chmod +x /bin/jq \
    && jq --version
...
RUN set -ex \
    && cd /usr/local \
    && curl --silent -L https://nodejs.org/dist/v${NODEJS_VERSION}/node-v${NODEJS_VERSION}-linux-x64.tar.xz | tar xJ \
    && cd / \
    && node --version
